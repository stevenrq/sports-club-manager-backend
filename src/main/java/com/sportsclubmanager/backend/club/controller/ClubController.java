...
public class ClubController {
    ...
    public ResponseEntity<List<ClubResponse>> getAll() {
        return ResponseEntity.ok(clubService.findAll().stream()
    ...
    @PreAuthorize("hasAnyRole('CLUB_ADMIN', 'ADMIN')")
    public ResponseEntity<Page<ClubResponse>> getAllPaginated(@PathVariable Integer page) {
        return ResponseEntity.ok(clubService.findAll(PageRequest.of(page, 5))
                .map(clubMapper::toClubResponse));
    }
