...
public class TournamentEventController {
    ...
    public ResponseEntity<ApiResponse<Tournament>> create(@Valid @RequestBody Tournament tournament, BindingResult bindingResult) {
        ...
        ApiResponse<Tournament> apiResponse = new ApiResponse<>(savedTournament);
    ...
    public ResponseEntity<Tournament> getById(@PathVariable Long id) {
        return tournamentEventService.findById(id)
    ...
    @PreAuthorize("hasAnyRole('CLUB_ADMIN', 'ADMIN')")
    public ResponseEntity<Page<Tournament>> getAllPaginated(@PathVariable Integer page) {
        return ResponseEntity.ok(tournamentEventService.findAll(PageRequest.of(page, 5)));
    }
    ...
    public ResponseEntity<ApiResponse<Tournament>> update(
        ...
        return tournamentEventService.update(id, tournamentEventUpdateRequest)
                .map(tournamentUpdated -> {
                    ApiResponse<Tournament> apiResponse = new ApiResponse<>(tournamentUpdated);
